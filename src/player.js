...
export class Player {
  ...
  get speed() {
    return this.character.stats.speed;
  }
  get jumpForce() {
    return this.character.stats.jumpForce;
  }
  get lightDamage() {
    return this.character.stats.lightDamage;
  }
  get heavyDamage() {
    return this.character.stats.heavyDamage;
  }
  ...
  isAttacking() {
    return (
      this.state === STATE.ATTACK_LIGHT || this.state === STATE.ATTACK_HEAVY
    );
  }
  ...
  getAttackHitbox() {
    ...
    const a = ATTACKS[this.attackType];
    const reach =
      a.reach + (this.weapon ? this.weapon.stats.reachBonus || 0 : 0);
    return {
  ...
  update(held, pressed, floorY, stageLeft, stageRight, platforms = []) {
    this.frameCount++;
    switch (this.state) {
      case STATE.IDLE:
      case STATE.WALK:
    ...
    // Collision: one-way platforms (land only when falling through platform top)
    let hitPlatform = false;
    if (Array.isArray(platforms) && platforms.length > 0 && this.vy >= 0) {
      const hurtboxLeft = this.x - this.w / 2;
      const hurtboxRight = this.x + this.w / 2;
      const prevFeetY = this.y - this.vy;
      const currFeetY = this.y;

      for (const platform of platforms) {
        if (!platform) continue;

        const horzOverlap =
          hurtboxRight > platform.x &&
          hurtboxLeft < platform.x + platform.width;
        const crossedTopFromAbove =
          prevFeetY <= platform.y && currFeetY >= platform.y;

        if (horzOverlap && crossedTopFromAbove) {
          this.y = platform.y;
          this.vy = 0;
          this.grounded = true;
    ...
    // Floor collision (fallback if no platform hit)
    if (!hitPlatform) {
      if (this.y >= floorY) {
        this.y = floorY;
        this.vy = 0;
    ...
    if (this.x - halfW < stageLeft) {
      this.x = stageLeft + halfW;
      this.vx = 0;
    }
    if (this.x + halfW > stageRight) {
      this.x = stageRight - halfW;
      this.vx = 0;
    }
  }
