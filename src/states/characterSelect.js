...
export class CharacterSelectState {
  ...
  update(game) {
    ...
    for (let p = 0; p < 2; p++) {
      ...
      if (
        pressed.right &&
        col < COLS - 1 &&
        this.cursors[p] + 1 < CHARACTERS.length
      )
        this.cursors[p]++;
      if (pressed.up && row > 0) this.cursors[p] -= COLS;
      if (pressed.down && this.cursors[p] + COLS < CHARACTERS.length)
        this.cursors[p] += COLS;
  ...
  draw(game, ctx) {
    ...
    if (this.confirmed[0] && this.confirmed[1]) {
      ...
      ctx.textAlign = "center";
      ctx.fillText("FIGHT!", W / 2, Math.min(H - 12, GRID_Y + 3 * CELL_H + 14));
    } else {
      ctx.fillStyle = "#555";
      ctx.font = "7px monospace";
      ctx.textAlign = "center";
      ctx.fillText(
        ...
        W / 2,
        Math.min(H - 10, GRID_Y + 3 * CELL_H + 12),
      );
    }
  }
