import { Player, ATTACKS, STATE } from '../player.js';
...
export class FightState {
  ...
  update(game) {
    ...
    this.p1.update(getHeld(1), getPressed(1), FLOOR_Y, STAGE_LEFT, STAGE_RIGHT);
    this.p2.update(getHeld(2), getPressed(2), FLOOR_Y, STAGE_LEFT, STAGE_RIGHT);
  ...
  resolveBodyCollision() {
    ...
    if (Math.abs(dx) < gap) {
      ...
      const dir  = dx < 0 ? -1 : 1;
      if (this.p1.state !== STATE.HURT && this.p1.state !== STATE.DEAD) this.p1.x -= push * dir;
      if (this.p2.state !== STATE.HURT && this.p2.state !== STATE.DEAD) this.p2.x += push * dir;
    }
  }
  ...
  drawWinOverlay(ctx, w, h) {
    ...
    const text  = this.winner === 'draw' ? 'DRAW!' : `PLAYER ${this.winner} WINS!`;
