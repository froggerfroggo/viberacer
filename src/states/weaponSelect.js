...
// 3-column grid with dynamic rows (supports 6 to 9 weapons cleanly)
const COLS = 3;
const CELL_W = 76;
const CELL_H = 58;
const GRID_X = Math.floor((480 - COLS * CELL_W) / 2); // 126

function gridRows() {
  return Math.max(1, Math.ceil(WEAPONS.length / COLS));
}
...
function gridY() {
  ...
  const topMin = 36;
  const topMax = 72;
  const centered = Math.floor((270 - rows * CELL_H) / 2);
  return Math.max(topMin, Math.min(topMax, centered));
}
...
export class WeaponSelectState {
  ...
  update(game) {
    ...
    for (let p = 0; p < 2; p++) {
      ...
      if (
        pressed.right &&
        col < COLS - 1 &&
        this.cursors[p] + 1 < WEAPONS.length
      )
        this.cursors[p]++;
      if (pressed.up && row > 0) this.cursors[p] -= COLS;
      if (pressed.down && this.cursors[p] + COLS < WEAPONS.length)
        this.cursors[p] += COLS;
  ...
  draw(game, ctx) {
    ...
    if (this.confirmed[0] && this.confirmed[1]) {
      ...
      ctx.textAlign = "center";
      ctx.fillText("FIGHT!", W / 2, topY + rows * CELL_H + 16);
    } else {
      ctx.fillStyle = "#555";
      ctx.font = "7px monospace";
      ctx.textAlign = "center";
      ctx.fillText("Press attack to confirm", W / 2, topY + rows * CELL_H + 14);
    }
  }
